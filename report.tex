...
The questions and possible answers are in the survey are listed in table-XX (draw the table of the questions and answers). We convert these answers to a preference vector. From the table, we can see that the values of each element of the vector are discrete. The questions list are far from complete. One potential future work is that we can add more questions based on users’ feedback in crowdsourcing.
...
\subsubsubsection{physicalPropertyDecisionTree}
...
\subsubsubsection{routeSuggestionDecisionTree}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.9\columnwidth]{pictures/Figure1}
